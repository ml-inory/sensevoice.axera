...
class SenseVoiceOnnx:
    def __init__(self, model_path, max_len=68, language="auto", use_itn=True, tokenizer=None, ban_emo_unk=True, is_dynamic=False):
        ...
        self.position_embed = SinusoidalPositionEncoder()
        ...
        self.language_query = np.load(f"{model_path_root}/{language}.npy")
        self.textnorm_query = np.load(f"{model_path_root}/withitn.npy") if use_itn else np.load(f"{model_path_root}/woitn.npy")
        self.event_emo_query = np.load(f"{model_path_root}/event_emo.npy")
    ...
    def choose_language(self, language):
        self.language_query = np.load(f"{self.model_path_root}/{language}.npy")
        self.input_query = np.concatenate((self.textnorm_query, self.language_query, self.event_emo_query), axis=1)
        self.language = language
    ...
    def infer_waveform(self, waveform: np.ndarray, language="auto"):
        ...
        if self.is_dynamic:
            # concat query
            sub_feat = np.concatenate([self.input_query, feat], axis=1)
            real_len = sub_feat.shape[1]

            masks = sequence_mask(np.array([real_len], dtype=np.int32), dtype=np.float32)
            position_encoding = self.position_embed.get_position_encoding(torch.from_numpy(sub_feat)).numpy()
